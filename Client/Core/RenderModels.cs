...
namespace CineKit
{
    public sealed partial class Plugin : BaseUnityPlugin
    {
        private void PrintLoadedMessage()
        {
            ...
            try
            {
                ...
                Console.WriteLine(
                    "[CineKit] Loaded - Insert menu, Home ESP");
            }
            finally
            {
            ...
            LogSource.LogInfo("CineKit loaded.");
        }
    ...
    internal sealed class ScopeRenderPass
    {
        ...
        public ScopeRenderPass(Camera camera, Font font)
        {
            ...
            _mesh = new Mesh
            {
                name = "CineKit Scope Mesh",
                hideFlags = HideFlags.HideAndDontSave
            };
            _mesh.MarkDynamic();
            _textMesh = new Mesh
            {
                name = "CineKit Scope Text Mesh",
                hideFlags = HideFlags.HideAndDontSave
            };
            _textMesh.MarkDynamic();
            _textMaterial = new Material(_font.material)
            {
                name = "CineKit Scope Text Material",
                hideFlags = HideFlags.HideAndDontSave
            };

            _command = new CommandBuffer
            {
                name = "CineKit Scope Pass"
            };
